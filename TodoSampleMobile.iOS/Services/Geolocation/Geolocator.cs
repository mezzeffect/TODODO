...
using CoreLocation;
using Foundation;
using TodoSampleMobile.iOS.Services.Geolocation;
...
namespace TodoSampleMobile.iOS.Services.Geolocation
{
    ...
    public class Geolocator : IGeolocator
    {
        ...
        public Task<Position> GetPositionAsync(int timeout, CancellationToken cancelToken, bool includeHeading)
        {
            ...
            if (!IsListening)
            {
                ...
                var singleListener = new GeolocationSingleUpdateDelegate(m, DesiredAccuracy, includeHeading, timeout,
                    cancelToken);
        ...
        {
            CLLocationManager locMgr = new CLLocationManager { PausesLocationUpdatesAutomatically = false };
            //new NSObject().InvokeOnMainThread(() => locMgr =);
        ...
        /// <summary>Convert miles to longitudinal degrees at a specific latitude.</summary>
        /// <returns>The longitudinal degrees at a specific latitude.</returns>
        /// <param name='miles'>miles</param>
        /// <param name='atLatitude'>atLatitude</param>
        ...
        public async Task<string> GetCountry(double latitude, double longitude)
        {
            try
            {
                ...
                //var srv =
                //    $"{Geolocation.Constants.GeoSrvcURL}{Constants.LatLongQuery}={latitude},{longitude}&{Constants.KeyQuery}={Constants.APIKey}"

                //await client.DownloadStringAsync(new Uri(srv));
                //var json = client.OpenReadAsync();
                HttpClient client = new HttpClient();
                // RestUrl = http://developer.xamarin.com:8081/api/todoitems{0}
                var uri =
                    new Uri(
                        string.Format(
                            $"{Constants.GeoSrvcURL}{Constants.LatLongQuery}={latitude},{longitude}&{Constants.KeyQuery}={Constants.APIKey}",
                            string.Empty));
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var root = JsonConvert.DeserializeObject<RvrsGeoRootobject>(content);
                    var addressComponnent = root.results.First().address_components.FirstOrDefault(r => r.types.Contains(Constants.Country));
                    if (addressComponnent != null)
                        return addressComponnent?.long_name;
                    return string.Empty;
                }
                else
                {
