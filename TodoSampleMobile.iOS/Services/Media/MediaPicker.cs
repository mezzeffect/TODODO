...
using TodoSampleMobile.iOS.Services.Media;
...
namespace TodoSampleMobile.iOS.Services.Media
{
    /// <summary>
    /// Class MediaPicker.
    /// </summary>
    public class MediaPicker : IMediaPicker
    {
        ...
        public MediaPicker()
        {
            ...
            var files = availableCameraMedia.Concat(availableLibraryMedia).ToList();
            if (files.Any(file => file == TypeMovie))
            {
                IsVideosSupported = true;
            }
            if (files.Any(file => file == TypeImage))
            {
        ...
        public Task<MediaFile> SelectVideoAsync(VideoMediaStorageOptions options)
        {
            ...
            return GetMediaAsync(UIImagePickerControllerSourceType.PhotoLibrary, TypeMovie, options);
        }
